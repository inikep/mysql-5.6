...
static int begin_packet_write_state(NET *net, uchar command,
  ...
  const bool include_command = (command < COM_END) || (command > COM_TOP_BEGIN);
  if (include_command) {
    ++total_len;
  }
...
static bool net_write_raw_loop(NET *net, const uchar *buf, size_t count) {
  ...
  while (count) {
    ...
    const ssize_t sentcnt = vio_write(net->vio, buf, count);
...
static bool net_read_raw_loop(NET *net, size_t count) {
  ...
  while (count) {
    const ssize_t recvcnt = vio_read(net->vio, buf, count);
