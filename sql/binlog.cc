...
MYSQL_BIN_LOG::read_gtids_from_binlog(const char *filename, Gtid_set *all_gtids,
                                      ...
                                      Tsid_map *tsid_map, bool verify_checksum,
...
bool MYSQL_BIN_LOG::find_first_log_not_in_gtid_set(char *binlog_file_name,
  ...
  for (auto rit = previous_gtid_set_map.rbegin();
    ...
    if (binlog_previous_gtid_set.is_subset(gtid_set)) {
      ...
      enum_read_gtids_from_binlog_status ret = read_gtids_from_binlog(
          full_log_name, NULL, NULL, first_gtid, gtid_set->get_tsid_map(),
  ...
  Gtid_set gtid_missing(gtid_set->get_tsid_map());
...
bool MYSQL_BIN_LOG::init_gtid_sets(
    ...
    bool need_lock,
    mysql::binlog::event::Transaction_boundary_parser *trx_parser,
    Gtid_monitoring_info *partial_trx, uint64_t *max_prev_hlc, bool startup) {
  char file_name_and_gtid_set_length[FILE_AND_GTID_SET_LENGTH];
  uchar *previous_gtid_set_in_file = NULL;
  ...
  Tsid_map *sid_map = NULL;
  if (all_gtids)
    sid_map = all_gtids->get_tsid_map();
  else if (lost_gtids)
    sid_map = lost_gtids->get_tsid_map();
...
#ifndef NDEBUG
        Gtid_set unused_lost_gtid(lost_gtids->get_tsid_map(),
                                  lost_gtids->get_tsid_map()->get_tsid_lock());
    ...
    if (enable_binlog_hlc && (!is_relay_log || raft_rotate_info)) {
      ...
      if (raft_rotate_info) {
        ...
        update_prev_gtid_and_opid(
            ...
            raft_rotate_info->rotate_opid.second, need_tsid_lock);
      }
    ...
    if (binlog_pos >=
      ...
      if (max_gtid_var != nullptr) {
        ...
        auto status [[maybe_unused]] =
            max_gtid.parse(global_tsid_map, max_gtid_var);
        global_tsid_lock->unlock();
      }
...
void MYSQL_BIN_LOG::report_missing_purged_gtids(
  ...
  DBUG_TRACE;
  THD *thd = current_thd;
  Gtid_set gtid_missing(lost_gtid_set->get_tsid_map());
...

  {
    ...
    while ((ev = binlog_file_reader.read_event_object())) {
      if (ev->get_type_code() == mysql::binlog::event::QUERY_EVENT &&
        ...
        in_transaction = true;
      } else if (ev->get_type_code() == mysql::binlog::event::QUERY_EVENT &&
                 !strcmp(static_cast<Query_log_event *>(ev)->query, "COMMIT")) {
        assert(in_transaction == true);
        in_transaction = false;
      } else if (is_any_gtid_event(ev)) {
        pending_gtid = true;
      } else if (ev->get_type_code() == mysql::binlog::event::XID_EVENT ||
      ...
      if (!(ev->get_type_code() == mysql::binlog::event::METADATA_EVENT &&
            pending_gtid)) {
        if (!in_transaction && !is_any_gtid_event(ev)) {
          valid_pos = binlog_file_reader.position();
          pending_gtid = false;
        }
      }
...
int get_committed_gtids(const std::vector<std::string> &gtids,
  ...
  for (const auto &gtid_s : gtids) {
    ...
    Gtid gtid;
    auto st = gtid.parse(global_tsid_map, gtid_s.c_str());
    if (st != mysql::utils::Return_status::ok) {
      global_tsid_lock->unlock();
      return (int)st;
    }
