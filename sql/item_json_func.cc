...
bool parse_json(const String &res, Json_dom_ptr *dom, bool require_str_or_json,
                ...
                const JsonErrorHandler &depth_handler, bool legacy_parsing) {
    ...
    *valid = !arg_item->val_json(&w);
    ...
    const bool failure = parse_json(
        *res, dom, require_str_or_json,
        ...
        JsonDepthErrorHandler, legacy_parsing);
    *valid = !failure;
    return parse_error;
  }
}
...
/**
   ...
   @param[in,out] final_str stores the final value in case the value is
               transformed.
   @param[in,out] value_holder String pointing either to final_str or to memory
                  area allocated inside value_holder.
...
longlong Item_func_json_contains::legacy_val_int() {
  ...
  */
  ...
  if (arg_count == 3) {
    ...
    if (parse_json(*value, &dom, true, parse_handler, JsonDepthErrorHandler,
                   true)) {
      return error_int();
    }
...
longlong Item_func_json_array_length::val_int() {
  ...
  if (wrapper.type() != enum_json_type::J_ARRAY) {
    String value;
    if (wrapper.to_string(&value, false, func_name(), JsonDepthErrorHandler)) {
      return error_int();
    }
...
String *Item_func_json_unquote::val_str(String *str) {
  ...
  try {
    ...
    */
    ...
    Json_dom_ptr dom;
    JsonParseDefaultErrorHandler parse_handler(func_name(), 0);
    if (parse_json(*utf8str, &dom, true, parse_handler, JsonDepthErrorHandler,
                   false)) {
      return error_str();
    }
  ...
  switch (m_cast_target) {
    ...
    case ITEM_CAST_JSON: {
      ...
      if (parse_json(*string_value, &default_value->json_default, true,
                     parse_handler, JsonDepthErrorHandler, false)) {
