...
/**
  ...
  @param counter_histogram_data  histogram data
  @param histogram_values        histogram values
*/
...
static int init_gtid_state_from_binlog() {
  Gtid_set executed_gtids_from_binlog(global_tsid_map, global_tsid_lock);
  Gtid_set lost_gtids_from_binlog(global_tsid_map, global_tsid_lock);
...
#endif
{
  ...
  */
  ...
  if (!opt_initialize && !opt_initialize_insecure && !enable_raft_plugin) {
    ...
    if (executed_gtids) {
      Checkable_rwlock *lock = executed_gtids->get_tsid_map()->get_tsid_lock();
      lock->wrlock();
      executed_gtids->to_string(&str, false, nullptr);
      lock->unlock();
    }
