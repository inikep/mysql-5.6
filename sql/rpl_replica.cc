...
int ReplicaInitializer::init_replica() {
  ...
  */
  ...
  /**
    ...
    global_tsid_map. However, idempotent recovery/apply uses
    rli->recovery_tsid_map. Hence rli->recovery_max_engine_gtid needs to be
    initialized by hashing into rli->recovery_tsid_map
    ...
    */
    ...
    MYSQL_BIN_LOG::enum_read_gtids_from_binlog_status ret =
        mysql_bin_log.read_gtids_from_binlog(
            ...
            NULL, NULL, global_tsid_map, opt_source_verify_checksum, false,
...
int rli_relay_log_raft_reset(
  ...
  */
  ...
  all_gtid_set->get_tsid_map()->get_tsid_lock()->wrlock();
  ...
  all_gtid_set->get_tsid_map()->get_tsid_lock()->unlock();
...
int raft_reset_slave(THD *) {
  ...
  */
  ...
  (const_cast<Gtid_set *>(mi->rli->get_gtid_set()))->clear_set_and_tsid_map();
...
static int exec_relay_log_event(THD *thd, Relay_log_info *rli,
  ...
  */
  ...
  if (ev) {
    ...
    if (enable_raft_plugin &&
        ev->get_type_code() == mysql::binlog::event::METADATA_EVENT) {
      Metadata_log_event *mev = static_cast<Metadata_log_event *>(ev);
      if (mev->does_exist(mysql::binlog::event::Metadata_event::
...
static int slave_start_single_worker(Relay_log_info *rli, ulong i) {
  ...
  if (!(w = Rpl_info_factory::create_worker(
            rli->is_fake() ? INFO_REPOSITORY_DUMMY : INFO_REPOSITORY_TABLE, i,
            rli, false))) {
