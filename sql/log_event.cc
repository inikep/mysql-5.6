...
Slave_worker *Log_event::get_slave_worker(Relay_log_info *rli) {
  ...
  if ((is_s_event = starts_group()) || is_any_gtid_event(this) ||
      get_type_code() == mysql::binlog::event::METADATA_EVENT ||
    ...
    } else if (get_type_code() == mysql::binlog::event::METADATA_EVENT) {
...
int Log_event::apply_event(Relay_log_info *rli) {
  ...
  if (opt_binlog_trx_meta_data &&
      get_type_code() == mysql::binlog::event::ROWS_QUERY_LOG_EVENT &&
      has_trx_meta_data()) {
      ...
      if (!parser_state.init(thd, thd->query().str, thd->query().length)) {
        ...
        if (sqlcom_can_generate_row_events(thd->lex->sql_command) &&
          ...
          if (option_name != nullptr) {
            rli->report(
                ERROR_LEVEL, ER_MTA_INCONSISTENT_DATA,
                "Source's binlog format is not ROW but %s is enabled on the "
                "replica, this should only be used when source's binlog format "
                "is ROW.",
                option_name);
...
/**
  ...
  @param table_def the table definition according to the master
...
static bool are_all_columns_signaled_for_key(TABLE *table, KEY *keyinfo,
  ...
  for (uint i = 0; i < keyinfo->actual_key_parts; i++) {
    if (table_def->use_column_names(table)) {
      Field *key_field = keyinfo->key_part[i].field;
      const std::pair<uint, bool> res =
          table_def->get_column_index(key_field->field_name);
...
INDEX_SCAN:
  ...
  */
  ...
  if (unlikely(potential_bi_inconsistency)) {
    ...
    thd->variables.gtid_next.to_string(global_tsid_map, gtid);
...
void Rows_log_event::set_readwriteset_from_col_names(TABLE *table,
  ...
  for (uint i = 0; i < tabledef->size(); ++i) {
    ...
    // handle read_set
    if ((get_general_type_code() == mysql::binlog::event::DELETE_ROWS_EVENT ||
         get_general_type_code() == mysql::binlog::event::UPDATE_ROWS_EVENT) &&
...
int Rows_log_event::force_write_to_binlog(table_def *tabledef,
  ...
  m_table->mark_columns_per_binlog_row_image(
      thd, get_general_type_code() == mysql::binlog::event::WRITE_ROWS_EVENT);
  ...
  while (m_curr_row != m_rows_end) {
    ...
    else if (event_type == mysql::binlog::event::UPDATE_ROWS_EVENT &&
             j++ % 2 == 1) {
      cols = &m_cols_ai;
      is_after_image = true;
    }

    if (event_type == mysql::binlog::event::UPDATE_ROWS_EVENT &&
        is_after_image) {
    ...
    */
    ...
    if (slave_run_triggers_for_rbr) {
      ...
      */
      ...
      for (Table_ref *tables = rli->tables_to_lock; tables;
    ...
    for (uint i = 0; ptr && (i < rli->tables_to_lock_count);
      ...
      */
      ...
      /*
        ...
        Table_ref (RPL_Table_ref) but row event uses only TABLE so I need to
        find somehow the corresponding Table_ref.
      */
      ptr->table->master_had_triggers =
          ((RPL_Table_ref *)ptr)->master_had_triggers;
    }
  ...
  if (table) {
    ...
    */
    switch (get_general_type_code()) {
      case mysql::binlog::event::DELETE_ROWS_EVENT:
        if (table_def->use_column_names(table))
          set_readwriteset_from_col_names(table, table_def, &m_cols);
        else
      ...
      case mysql::binlog::event::UPDATE_ROWS_EVENT:
        if (table_def->use_column_names(table))
          set_readwriteset_from_col_names(table, table_def, &m_cols_ai);
        else {
        ...
        if (!is_any_column_signaled_for_table(
                table, table_def, &this->m_local_cols_ai, &m_cols_ai))
    ...
    error = do_before_row_operations(rli, table_def);
    ...
    */
    ...
    if ((m_rows_lookup_algorithm != ROW_LOOKUP_NOT_NEEDED) &&
        !is_any_column_signaled_for_table(table, table_def, &this->m_local_cols,
                                          &m_cols)) {
    ...
    do {
      ...
      error = (this->*do_apply_row_ptr)(rli, table_def);
  ...
  AFTER_MAIN_EXEC_ROW_LOOP:
    ...
    */
    ...
    if (m_force_binlog_idempotent && !error) {
      ...
      error =
          force_write_to_binlog(table_def, const_cast<Relay_log_info *>(rli));
    }
    // reset back the db
    thd->reset_db(current_db_name_saved);
  }  // if (table)
...
bool Table_map_log_event::init_fb_format_pk_fields() {
  ...
  if (m_primary_key_fields_size) {
    m_primary_key_fields = static_cast<unsigned char *>(
        mysql::binlog::event::bapi_malloc(m_primary_key_fields_size, 0));
...
bool Table_map_log_event::init_fb_format_sign_bits() {
  if (m_sign_bits_size * 8 >= m_table->s->fields) {
    ...
    m_sign_bits = static_cast<unsigned char *>(
        mysql::binlog::event::bapi_malloc(m_sign_bits_size, 0));
    if (!m_sign_bits) {
      return true;
    }
...
bool Table_map_log_event::init_fb_format_col_names() {
  ...
  if (m_column_names_size) {
    ...
    m_column_names = static_cast<unsigned char *>(
        mysql::binlog::event::bapi_malloc(m_column_names_size, 0));
    if (!m_column_names) {
      return true;
    }
  }
...
int Write_rows_log_event::do_before_row_operations(
    const Relay_log_info *const rli, table_def *table_def) {
  int error = 0;
  ...
  */
   ...
   */
  decide_row_lookup_algorithm_and_key(table_def);
...
int Delete_rows_log_event::do_before_row_operations(const Relay_log_info *const,
                                                    table_def *table_def) {
  int error = 0;
  DBUG_TRACE;
  ...
  */
  ...
  error = row_operations_scan_and_key_setup(table_def);
  return error;
}
...
Metadata_log_event::Metadata_log_event(
    char const *buffer,
    mysql::binlog::event::Format_description_event const *description_event)
    : mysql::binlog::event::Metadata_event(buffer, description_event),
      Log_event(header(), footer()) {}
...
bool Metadata_log_event::write_data_body(Basic_ostream *ostream) {
  ...
  if (write_raft_ingestion_checkpoint(ostream)) DBUG_RETURN(1);
...
bool Metadata_log_event::write_raft_ingestion_checkpoint(
    Basic_ostream *ostream) {
  ...
  char buffer[ENCODED_RAFT_INGESTION_CHECKPOINT_SIZE];
  char *ptr_buffer = buffer;

  if (write_type_and_length(
          ostream, Metadata_event_types::RAFT_INGESTION_CHECKPOINT_TYPE,
          sizeof(int64_t) * 2)) {
    DBUG_RETURN(1);
  }

  int8store(ptr_buffer, raft_ingestion_checkpoint_.first);
  ptr_buffer += sizeof(raft_ingestion_checkpoint_.first);

  int8store(ptr_buffer, raft_ingestion_checkpoint_.second);
  ptr_buffer += sizeof(raft_ingestion_checkpoint_.second);
...
bool Metadata_log_event::write_raft_ingestion_upper_bound(
    Basic_ostream *ostream) {
  ...
  char buffer[ENCODED_RAFT_INGESTION_UPPER_BOUND_SIZE];
  char *ptr_buffer = buffer;

  if (write_type_and_length(
          ostream, Metadata_event_types::RAFT_INGESTION_UPPER_BOUND_TYPE,
          sizeof(raft_ingestion_upper_bound_))) {
    DBUG_RETURN(1);
  }

  int8store(ptr_buffer, raft_ingestion_upper_bound_);
  ptr_buffer += ENCODED_RAFT_INGESTION_UPPER_BOUND_SIZE;
      ...
      if (ev->begin_event()) {
        assert(rli->curr_group_seen_gtid ||
               ev->begin_event()->raw_event()->get_type_code() ==
                   mysql::binlog::event::INTVAR_EVENT ||
               ev->begin_event()->raw_event()->get_type_code() ==
                   mysql::binlog::event::RAND_EVENT ||
               ev->begin_event()->raw_event()->get_type_code() ==
                   mysql::binlog::event::USER_VAR_EVENT ||
               ev->begin_event()->raw_event()->get_type_code() ==
                   mysql::binlog::event::BEGIN_LOAD_QUERY_EVENT ||
               ev->begin_event()->raw_event()->get_type_code() ==
                   mysql::binlog::event::APPEND_BLOCK_EVENT ||
               is_ignorable_event());
        assert(submode->prev_event != nullptr);
        ev->begin_event()->whole_group_scheduled = true;
      } else {
...
bool Rows_log_event::get_keys(Relay_log_info *rli,
  ...
  RPL_Table_ref *table_list = nullptr;
  void *memory = nullptr;
  bool success = false;
...
bool Rows_log_event::get_table_ref(Relay_log_info *rli, void **memory,
  ...
  bool error = get_table_from_cache(thd, (Table_ref *)(*table_list));
  assert(thd->mdl_context.has_locks());
  DBUG_RETURN(error);
}
...
*/
void Rows_log_event::close_table_ref(RPL_Table_ref *table_list) {
  DBUG_ENTER("Rows_log_event::close_table_ref");
  assert(thd->mdl_context.has_locks());

  return_table_to_cache(thd, (Table_ref *)table_list);
...
void *Table_map_log_event::setup_table_rli(RPL_Table_ref **table_list) {
  ...
  if (!(memory = my_multi_malloc(PSI_NOT_INSTRUMENTED, MYF(MY_WME), table_list,
                                 (uint)sizeof(RPL_Table_ref), &db_mem,
