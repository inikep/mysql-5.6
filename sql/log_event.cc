...
int Rows_log_event::force_write_to_binlog(Relay_log_info *rli) {
  ...
  while (m_curr_row != m_rows_end) {
    ...
    else if (event_type == mysql::binlog::event::UPDATE_ROWS_EVENT && i++ % 2 == 1) {
      cols = &m_cols_ai;
      is_after_image = true;
    }
  ...
  if (table) {
    ...
    */
    ...
    if (rli->last_gtid[0] != '\0' && thd->is_enabled_idempotent_recovery() &&
      ...
      Gtid current_gtid;
      global_tsid_lock->rdlock();
      auto status = current_gtid.parse(global_tsid_map, rli->last_gtid);
      global_tsid_lock->unlock();

      if (status == mysql::utils::Return_status::ok &&
          current_gtid.sidno == rli->recovery_max_engine_gtid.sidno &&
