...
std::pair<my_off_t, int> Binlog_sender::get_binlog_end_pos(
  ...
  if (unlikely(!wait_for_ack &&
               !raw_log->is_active(m_linfo.log_file_name))) {
...
int Binlog_sender::wait_new_events(my_off_t log_pos) {
  ...
  */
  ...
  m_thd->ENTER_COND(raw_log->get_log_cond(),
                    raw_log->get_binlog_end_pos_lock(),
...
bool Binlog_sender::stop_waiting_for_update(my_off_t log_pos, MYSQL_BIN_LOG *raw_log) const {
  bool wait_for_ack = !m_is_semi_sync_slave;
...
inline int Binlog_sender::wait_with_heartbeat(MYSQL_BIN_LOG *raw_log,
  ...
  while (!stop_waiting_for_update(log_pos, raw_log)) {
    ...
    raw_log->wait_for_update(m_heartbeat_period);

    if (stop_waiting_for_update(log_pos, raw_log)) {
      return 0;
    }
    raw_log->unlock_binlog_end_pos();
    Scope_guard lock([&]() { raw_log->lock_binlog_end_pos(); });
#ifndef NDEBUG
