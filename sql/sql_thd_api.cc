...
char *thd_security_context_internal(THD *thd, char *buffer, size_t length,
  ...
  */
  ...
  if (thd->query().str) {
    ...
    str.append('\n');
    str.append(query_str, truncated_str_length(
                              thd->charset(), query_str, query_len,
                              (max_query_len < 1) ? query_len : max_query_len));
  }
...
bool tp_enqueue_task(tp_routine routine, void *param,
  ...
  MYSQL_CALLBACK_RETURN(Connection_handler_manager::cpu_scheduler_functions,
                        enqueue_task, (routine, param, tenant_id, hint));
  return false;
}
