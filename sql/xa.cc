...
int ha_recover(xid_to_gtid_container *commit_list, Xa_state_list *xa_list,
  ...
  */
  ...
  if (info.binlog_max_gtid && !info.binlog_max_gtid->is_empty()) {
    ...
    info.binlog_max_gtid->to_string(global_tsid_map, max_gtid_buf);
    if (!info.binlog_smallest_max_gtid->is_empty()) {
      info.binlog_smallest_max_gtid->to_string(global_tsid_map,
...
int ha_update_binlog_pos(const char *binlog_file, my_off_t binlog_pos,
  ...
  if (max_gtid && !max_gtid->is_empty()) {
    global_tsid_lock->rdlock();
    max_gtid->to_string(global_tsid_map, info.binlog_max_gtid_buf);
    global_tsid_lock->unlock();
  }
