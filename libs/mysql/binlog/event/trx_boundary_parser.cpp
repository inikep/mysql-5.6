...
bool Transaction_boundary_parser::check_row_logging_constraints(
  ...
  if (EVENT_BOUNDARY_TYPE_PRE_STATEMENT == m_current_boundary_state) {
    ...
    return log_event_info.event_type != mysql::binlog::event::USER_VAR_EVENT ||
  ...
  if (log_event_info.event_type ==
          mysql::binlog::event::BEGIN_LOAD_QUERY_EVENT ||
      log_event_info.event_type ==
          mysql::binlog::event::EXECUTE_LOAD_QUERY_EVENT ||
      log_event_info.event_type == mysql::binlog::event::APPEND_BLOCK_EVENT ||
      log_event_info.event_type == mysql::binlog::event::DELETE_FILE_EVENT)
    return true;

  // DML using non row events
  if (EVENT_BOUNDARY_TYPE_STATEMENT == m_current_boundary_state &&
      current_parser_state == EVENT_PARSER_DML)
    if (log_event_info.event_type != mysql::binlog::event::TABLE_MAP_EVENT &&
        log_event_info.event_type != mysql::binlog::event::WRITE_ROWS_EVENT &&
        ...
        log_event_info.event_type != mysql::binlog::event::VIEW_CHANGE_EVENT) {
      // We allow XA control statements
      if (log_event_info.event_type == mysql::binlog::event::QUERY_EVENT) {
