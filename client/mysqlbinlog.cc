...
static Exit_status safe_connect() {
  ...
  */
  ...
  mysql_options(mysql_handle, MYSQL_OPT_NET_RECEIVE_BUFFER_SIZE,
...
static Exit_status dump_remote_log_entries(PRINT_EVENT_INFO *print_event_info,
  ...
  */
  ...
  if (opt_use_dscp && mysql_query(mysql_handle, "SET SESSION dscp_on_socket=9")) {
    error(
...
inline bool gtid_client_init() {
  const bool res = (!(global_tsid_lock = new Checkable_rwlock) ||
              ...
              !(gtid_set_stop = new Gtid_set(global_tsid_map)));
  if (res) {
    gtid_client_cleanup();
  }
  return res;
