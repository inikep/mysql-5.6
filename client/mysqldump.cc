...
static void write_master_status_to_output(const char *filename,
  ...
  fprintf(
      ...
      get_compatible_rpl_replica_command("CHANGE REPLICATION SOURCE TO")
          .c_str(),
      get_compatible_rpl_replica_command("SOURCE_LOG_FILE").c_str(), filename,
      get_compatible_rpl_replica_command("SOURCE_LOG_POS").c_str(), position);
  check_io(md_result_file);
}
  ...
  if (result_length) {
    ...
    while (nullptr != (row = mysql_fetch_row(res))) {
      ...
      if (row[4] && *row[4]) order_by_part = quote_name(row[4], buff, false);
        ...
        continue;
      if (end != result) end = my_stpcpy(end, desc ? " DESC," : ",");
      if (braces_length)
  ...
  */
  ...
  if (opt_source_data) {
    if (bin_log_name[0] && bin_log_pos[0]) {
      ...
      if (do_show_binary_log_status(mysql)) goto err;
    }
  }
